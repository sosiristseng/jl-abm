...
function create_params(;
	...
	## Migrate to self (no migration) set to 1
...
# Stepping functions in the SIR ABM
...
# Observable are more efficient than updating the whole figure.
...
# Connect (lift) model observables to modle properties
fracs = lift(infected_fractions, abmobs.model)
color = lift(fs -> [cgrad(:inferno)[f] for f in fs], fracs)
title = lift(
    (m) -> "step = $(abmtime(model)), infected = $(round(Int, 100infected_fraction(m, allids(m))))%",
    abmobs.model
)
...
# Run the model for 50 steps and collect the data
...
li = lines!(ax, x, log10.(data[:, :infected_status]), color = :blue)
lr = lines!(ax, x, log10.(data[:, :recovered_status]), color = :red)
dead = log10.(N .- data[:, :length_status])
